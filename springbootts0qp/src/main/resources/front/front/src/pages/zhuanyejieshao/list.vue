...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"width":"100%","padding":"10px","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
      <el-form-item :style='{"margin":"5px 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>学校名称</div>
        <el-select v-model="formSearch.xuexiaomingcheng" placeholder="请选择学校名称" :clearable="true">
          <el-option v-for="(item, index) in xuexiaomingchengOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
	  ...
	  <el-button v-if="isAuth('zhuanyejieshao','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#d3ffe4","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/zhuanyejieshaoAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","display":"flex","height":"auto"}'>
			<div :style='{"padding":"5px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"10px 1%","background":"#ffff","flexDirection":"row-reverse","display":"flex","width":"31%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div class="item-info" :style='{"width":"280px","padding":"10px","overflow":"hidden","flexDirection":"column","display":"flex","height":"auto"}'>
					<div :style='{"lineHeight":"20px","fontSize":"14px","overflow":"hidden","color":"#333","height":"20px"}' class="name ">{{item.xuexiaomingcheng}}</div>
					<div :style='{"lineHeight":"20px","fontSize":"14px","overflow":"hidden","color":"#333","height":"20px"}' class="name ">{{item.zhuanyemingcheng}}</div>
					<div :style='{"lineHeight":"20px","fontSize":"14px","overflow":"hidden","color":"#333","height":"20px"}' class="name ">{{item.zhuanyeleibie}}</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/xuexiaoxinxi/xuexiaomingcheng').then(res => {
        if (res.data.code == 0) {
          this.xuexiaomingchengOptions = res.data.data;
        }
      });
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.xuexiaomingcheng != '') searchWhere.xuexiaomingcheng = this.formSearch.xuexiaomingcheng;
        if (this.formSearch.zhuanyemingcheng != '') searchWhere.zhuanyemingcheng = '%' + this.formSearch.zhuanyemingcheng + '%';
        this.$http.get('zhuanyejieshao/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/zhuanyejieshaoDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
